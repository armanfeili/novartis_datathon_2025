...
from .validation import (
    create_validation_split, get_fold_series,
    create_stratified_group_kfold, validate_cv_splits  # Standardized CV
)
...
def run_cross_validation(
    ...
    Args:
        ...
        cv_strategy: CV strategy to use:
            - 'stratified_group': New standardized CV (create_stratified_group_kfold) - DEFAULT
            - 'sklearn_stratified': Original sklearn StratifiedKFold (get_fold_series)
            - 'grouped': GroupKFold by therapeutic area (get_grouped_kfold_series)
    ...
    # Get folds based on cv_strategy toggle
    if cv_strategy == 'stratified_group':
        # New standardized CV with bucket stratification and series grouping
        # Returns (train_idx, val_idx, fold_info) - we'll convert to DataFrames
        fold_data = create_stratified_group_kfold(panel_features, n_folds=n_folds, random_state=seed)
        folds = []
        for train_idx, val_idx, _ in fold_data:
            # Use .loc because train_idx/val_idx are actual index values, not positional
            train_df = panel_features.loc[train_idx].copy()
            val_df = panel_features.loc[val_idx].copy()
            folds.append((train_df, val_df))
        logger.info(f"Using cv_strategy='stratified_group' (create_stratified_group_kfold)")
    elif cv_strategy == 'sklearn_stratified':
        # Original sklearn-based StratifiedKFold
        folds = get_fold_series(panel_features, n_folds=n_folds, random_state=seed)
        logger.info(f"Using cv_strategy='sklearn_stratified' (get_fold_series)")
    elif cv_strategy == 'grouped':
        # GroupKFold by therapeutic area
        from .validation import get_grouped_kfold_series
        folds = get_grouped_kfold_series(panel_features, n_folds=n_folds, random_state=seed)
        logger.info(f"Using cv_strategy='grouped' (get_grouped_kfold_series)")
    else:
        raise ValueError(f"Unknown cv_strategy: {cv_strategy}. Use 'stratified_group', 'sklearn_stratified', or 'grouped'")
...
def run_sweep_experiments(
    ...
    if collect_summary and all_metrics:
        ...
        with open(summary_dir / "sweep_results.md", "w") as f:
            ...
            f.write(f"- **Configs Swept**: {len(sweep_runs)}\n\n")
