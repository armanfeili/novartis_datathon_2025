...
class HybridPhysicsMLModel:
    ...
    def __init__(
        ...
        Args:
            ...
            monotonic_constraints: Dict of {feature_name: direction} or 'auto'
        """
    ...
    def _get_monotonic_constraints_array(self, feature_names: List[str]) -> Optional[List[int]]:
        """Build monotonic constraints array for feature list."""
        if self.monotonic_constraints is None or self.monotonic_constraints == False:
            return None
        
        if isinstance(self.monotonic_constraints, dict):
            constraint_dict = self.monotonic_constraints
        elif self.monotonic_constraints == 'auto' or self.monotonic_constraints == True:
        ...
        if any(c != 0 for c in constraints):
            logger.info(f"Hybrid model: applying monotonic constraints to {sum(c != 0 for c in constraints)} features")
            return constraints
        return None
    ...
    def _create_ml_model(self, feature_names: Optional[List[str]] = None):
        """Create the ML model instance with optional monotonic constraints."""
        params = self.params.copy()
        
        # Apply monotonic constraints if specified
        if feature_names is not None:
            constraints = self._get_monotonic_constraints_array(feature_names)
    ...
    def fit(
        ...
        # Step 3: Train ML model on residuals (pass feature names for monotonic constraints)
        self.ml_model = self._create_ml_model(feature_names=self.feature_names)
