...
def main():
    ...
    def load_model(path: str):
        ...
        try:
            ...
            # Handle dict-wrapped models (e.g., {'model': model, 'feature_names': [...], 'params': {...}})
            if isinstance(loaded, dict) and 'model' in loaded:
                logger.info(f"Unwrapping model from dict (keys: {list(loaded.keys())})")
                raw_model = loaded['model']
                feature_names = loaded.get('feature_names', [])
                params = loaded.get('params', {})
                
                # Check if it's an XGBoost Booster - wrap it for DataFrame prediction
                import xgboost as xgb
                if isinstance(raw_model, xgb.Booster):
                    logger.info("Wrapping XGBoost Booster for DataFrame prediction")
                    # Create a wrapper that handles DMatrix conversion
                    class XGBBoosterWrapper:
                        ...
                        def predict(self, X):
                            import pandas as pd
                            X_proc = X.copy() if isinstance(X, pd.DataFrame) else pd.DataFrame(X)
                            # Convert categorical columns to numeric codes
                            for col in X_proc.columns:
                                if X_proc[col].dtype.name == 'category':
                                    X_proc[col] = X_proc[col].cat.codes
                            
                            # Reorder columns to match training feature order if needed
                            if self.feature_names:
                                # Only keep features that exist in the model
                                available_features = [f for f in self.feature_names if f in X_proc.columns]
                                missing_features = [f for f in self.feature_names if f not in X_proc.columns]
                                if missing_features:
                    ...
                    return XGBBoosterWrapper(raw_model, feature_names)
                
                # Check if it's a LightGBM Booster
                try:
                    import lightgbm as lgb
                    if isinstance(raw_model, lgb.Booster):
                        logger.info("Wrapping LightGBM Booster for DataFrame prediction")
                        ...
                        return LGBBoosterWrapper(raw_model, feature_names)
                except ImportError:
                    pass
