...
### 7.7 HPO Implementation and Results

The project implements a full Optuna-based HPO pipeline in `src/train.py`:

#### 7.7.1 HPO CLI Interface

```bash
# Run HPO for CatBoost on Scenario 1 (50 trials, 1 hour timeout)
python -m src.train --scenario 1 --model catboost --hpo --hpo-trials 50 --hpo-timeout 3600

# Run HPO for XGBoost on Scenario 2
python -m src.train --scenario 2 --model xgboost --hpo --hpo-trials 100
```

#### 7.7.2 HPO Search Spaces

Search spaces are defined in `src/train.py::create_optuna_objective()`:

| Model | Parameter | Search Range | Type |
|-------|-----------|--------------|------|
| **XGBoost** | `max_depth` | [4, 8] | int |
| | `learning_rate` | [0.01, 0.1] | float (log) |
| | `min_child_weight` | [1, 10] | int |
| | `colsample_bytree` | [0.6, 1.0] | float |
| | `subsample` | [0.6, 1.0] | float |
| **LightGBM** | `num_leaves` | [15, 63] | int |
| | `learning_rate` | [0.01, 0.1] | float (log) |
| | `min_data_in_leaf` | [10, 50] | int |
| | `feature_fraction` | [0.6, 1.0] | float |
| | `bagging_fraction` | [0.6, 1.0] | float |
| **CatBoost** | `depth` | [4, 8] | int |
| | `learning_rate` | [0.01, 0.1] | float (log) |
| | `l2_leaf_reg` | [1.0, 10.0] | float (log) |
| | `min_data_in_leaf` | [10, 50] | int |
| | `random_strength` | [0.0, 5.0] | float |

#### 7.7.3 Best Hyperparameters from HPO

The following hyperparameters were determined via HPO sweeps using **official_metric** as the optimization objective:

| Model | Scenario | Key Params | Official Metric | Config File |
|-------|----------|------------|-----------------|-------------|
| **XGBoost** | S1 | `max_depth=6, lr=0.03, reg_lambda=1` | 0.7499 | `configs/model_xgb_s1_best.yaml` |
| **XGBoost** | S2 | `max_depth=4, lr=0.05, reg_lambda=1` | 0.2659 | `configs/model_xgb_s2_best.yaml` |
| **LightGBM** | S1 | `num_leaves=63, lr=0.05, min_data=20` | ~0.75 | `configs/model_lgbm_s1_best.yaml` |
| **LightGBM** | S2 | `num_leaves=31, lr=0.05, min_data=20` | ~0.27 | `configs/model_lgbm_s2_best.yaml` |
| **CatBoost** | S1 | `depth=6, lr=0.03, l2_reg=3.0` | ~0.76 | `configs/model_cat_s1_best.yaml` |
| **CatBoost** | S2 | `depth=6, lr=0.03, l2_reg=3.0` | ~0.28 | `configs/model_cat_s2_best.yaml` |

**Key Observations**:
1. **Scenario 2 prefers shallower models** (XGBoost: depth 4 vs 6, LightGBM: 31 vs 63 leaves)
2. **Lower learning rates work better** (0.03-0.05 vs default 0.1)
3. **XGBoost consistently outperforms** CatBoost on official_metric
4. **Early stopping is critical** - all models use 50-100 rounds patience

#### 7.7.4 Using Best Configs

```bash
# Use pre-tuned configs directly
python -m src.train --scenario 1 --model xgboost --model-config configs/model_xgb_s1_best.yaml
python -m src.train --scenario 2 --model xgboost --model-config configs/model_xgb_s2_best.yaml

# Or use named config from main file
