...
# 6. Generate predictions (use actual artifact folder names from training output)
python -m src.inference --model-s1 artifacts/<TIMESTAMP>_catboost_scenario1/model_1.bin \
                         --model-s2 artifacts/<TIMESTAMP>_catboost_scenario2/model_2.bin \
...
```bash
# Use the actual model paths from your training output
python -m src.inference \
    --model-s1 artifacts/<TIMESTAMP>_catboost_scenario1/model_1.bin \
    --model-s2 artifacts/<TIMESTAMP>_catboost_scenario2/model_2.bin \
    --output submissions/submission.csv \
...
# 2. Use the same seed
python -m src.train --scenario 1 --model catboost --model-config configs/model_cat.yaml --seed 42
...
python -m src.train \
    ...
    --model-config configs/model_cat.yaml \
    --seed 42 \
    --run-name catboost_s2

# Generate submission (use the run-name folders)
python -m src.inference \
    --model-s1 artifacts/catboost_s1/model_1.bin \
    --model-s2 artifacts/catboost_s2/model_2.bin \
